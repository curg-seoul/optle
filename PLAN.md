...
| 결제 네트워크 | **base-sepolia (현재)** / Mantle은 미해결 | ⚠️ **x402 v1.2 `Network` enum에 Mantle 없음**(base/base-sepolia/avalanche/polygon/sei/solana 등만 지원). 데모는 base-sepolia + x402 기본 테스트넷 facilitator로 동작. Mantle은 custom-chain 설정 + 자체 facilitator 필요(§9). |
...
PAYMENT_PRICE=$0.01             # 소액 (x402가 네트워크별 토큰으로 환산)
PAYMENT_NETWORK=base-sepolia    # ⚠️ x402 v1.2는 Mantle 미지원 (§3/§9 참고)
FACILITATOR_URL=                # 비우면 x402 기본 테스트넷 facilitator 사용

# 실제 적용된 서버 env는 apps/server/.env.example 참고
...
- ⏳ **M4 — x402 결제 게이트** — `x402-express`로 `/api/optimize`를 결제 보호. *키/네트워크 없어 실행 테스트는 스킵, 코드만 작성.*
...
| 리스크 | 대응 |
|------|------|
| **x402가 Mantle을 지원하지 않음 (확인됨)** | x402 v1.2 `Network` enum에 Mantle 부재. **현재: base-sepolia로 데모.** Mantle 필요 시 옵션: (a) x402 custom-chain(eip155) 설정 + Mantle USDC/스테이블 주소·decimals 지정, (b) Mantle을 검증하는 facilitator 자체 호스팅. 해카톤이 Mantle 결제를 필수로 요구하면 이걸 별도 스파이크로. |
